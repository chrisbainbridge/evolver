...
def main():
    ...
    # parse command line
    try:
        opts, args = getopt.getopt(sys.argv[1:], 'cdr:ebg:hi:k:ln:p:q:sz:t:uvm', ['qt=','topology=','update=','node_type=','nodes=','dom_bias=','dom_value=','dom_weight=','ga-elite', 'ga-steady-state','mutation-rate=','gauss-noise=','nodes_per_input=','network=','plotbpg=','plotfitness=','plotnets=','unroll','nb_dist=','toponly','movie=','sim=','states=', 'fitness='])
    ...
    if plotfitness:
        ...
        fdat.close()
        f = open(fbase + '.gplt', 'w')
        s = """# autogenerated gnuplot script
        set style data line
        set terminal pdf
        set output "%s"
        ...
        if ext != 'gplt':
            assert ext == 'pdf'
            os.system('gnuplot %s'%(fbase + '.gplt'))
            os.remove(fbase + '.gplt')
            if view:
